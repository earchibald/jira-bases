...
export interface CommentIssueSuggestModalOptions {
  app: App;
  client: JiraClient;
...
export class CommentIssueSuggestModal extends SuggestModal<Issue> {
  private readonly client: JiraClient;
  private readonly frontmatterKeys: Set<string>;
  private readonly onChooseIssue: (issue: Issue) => void;
  private debounceTimer: ReturnType<typeof setTimeout> | null = null;
  private requestSeq = 0;
  private reportedKinds = new Set<string>();

  constructor(opts: CommentIssueSuggestModalOptions) {
    super(opts.app);
    this.client = opts.client;
    this.frontmatterKeys = new Set(opts.frontmatterKeys);
  ...
  getSuggestions(query: string): Promise<Issue[]> {
    if (!query.trim()) return this.fetchFrontmatterIssues();
  ...
  private async fetchFrontmatterIssues(): Promise<Issue[]> {
    const keys = [...this.frontmatterKeys];
  ...
  private fetchByText(query: string, seq: number): Promise<Issue[]> {
    ...
    return new Promise((resolve) => {
      this.debounceTimer = setTimeout(async () => {
        ...
        resolve(this.elevate(result.value));
      }, DEBOUNCE_MS);
    });
  }

  private elevate(issues: Issue[]): Issue[] {
    const elevated: Issue[] = [];
    const rest: Issue[] = [];
    for (const issue of issues) {
      if (this.frontmatterKeys.has(issue.key)) {
        elevated.push(issue);
      } else {
        rest.push(issue);
      }
